...
export async function githubAppMiddleWare(req, res, next) {
    ...
    const { token, refresh_token, expires_in, refresh_token_expires_in } = githubRecord;
    const now = new Date();
    ...
    if (now >= expiresInDate && now < refreshExpiresInDate) {
        try {
            ...
            const { access_token, refresh_token: new_refresh_token, expires_in, refresh_token_expires_in, } = response.data;
